...
import com.sonar.csharp.plugin.project.VisualUtils;
import com.sonar.csharp.plugin.resource.CSharpFile;
import com.sonar.csharp.plugin.resource.CSharpFileLocator;
 ...
 */
public class CSharpSourceImporterDOTNET extends AbstractSourceImporter {

  private final static Logger log = LoggerFactory.getLogger(CSharpSourceImporterDOTNET.class);
   ...
   */
  public CSharpSourceImporterDOTNET(CSharp cSharp) {
    super(cSharp);
    this.fileLocator = new CSharpFileLocator();
  }
  ...
  private void parseVisualProject(VisualStudioProject visualStudioProject, SensorContext context, Project project) {
    ...
    boolean excludeGeneratedCode = project.getConfiguration().getBoolean(CSharpConstants.SONAR_EXCLUDE_GEN_CODE_KEY, true);
