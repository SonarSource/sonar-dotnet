...
public class CSharpSonarRulesDefinition implements RulesDefinition {
  ...
  public void define(Context context) {
    ...
    repository.createRule("S1871").setName("Two branches in the same conditional structure should not have exactly the same implementation").setSeverity(Severity.MAJOR);
    repository.createRule("S2681").setName("Multiline blocks should be enclosed in curly braces").setSeverity(Severity.CRITICAL);
    ...
    repository.createRule("S1697").setName("Short-circuit logic should be used to prevent null pointer dereferences in conditionals").setSeverity(Severity.BLOCKER);
    repository.createRule("S1994").setName("\"for\" loop incrementers should modify the variable being tested in the loop's stop condition").setSeverity(Severity.CRITICAL);
