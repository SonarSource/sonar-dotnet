...
public class SourceMonitorResultParser {
	...
	private final static Logger log = LoggerFactory.getLogger(SourceMonitorResultParser.class);
	...
	private MethodMetric generateMethod(Element methodNode, File file) {
		...
		if (rawName.endsWith(".get()") || rawName.endsWith(".set()")) {
			...
			methodName = StringUtils.removeEnd(StringUtils.substringAfterLast(
			    rawName, "."), "()")
			    + "_" + propertyName + "()";
		} else {
