...
public class FxCopSensorTest {
  ...
  public void testShouldExecuteOnProjectWithSkip() {
    Project project = mock(Project.class);
    Configuration configuration =  mock(Configuration.class);
    when(configuration.getString(FXCOP_MODE_KEY, FXCOP_DEFAULT_MODE)).thenReturn(FXCOP_SKIP_MODE);
    when(project.getPackaging()).thenReturn("sln");
    when(project.getConfiguration()).thenReturn(configuration);
    assertFalse(sensor.shouldExecuteOnProject(project)); 
  }
  ...
  private void testAnalyseReuse(String reportPathParam) {
    ...
    Configuration configuration =  mock(Configuration.class);
    when(configuration.getString(FXCOP_MODE_KEY, FXCOP_DEFAULT_MODE)).thenReturn(FXCOP_REUSE_MODE);
    when(configuration.getString(FXCOP_REPORT_KEY)).thenReturn(reportPathParam);
    when(project.getConfiguration()).thenReturn(configuration);
  ...
  public void testAnalyseReuseSameDir() {
    testAnalyseReuse("fxcop-alt-report.xml");
  }

  @Test
  public void testAnalyseReuseDifferentDir() {
    testAnalyseReuse("../working/fxcop-report.xml;../working/fxcop-sl-report.xml");
  }
