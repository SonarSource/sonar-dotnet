...
public class GallioSensor extends AbstractDotnetSensor
{
  ...
  private void collect(Project project, File report, SensorContext context)
  {
    ...
    for (UnitTestReport testReport : reports)
    {
      ...
      CSharpFile testFile = CSharpFile.from(project, testReport.getSourceFile(), true);
   ...
   */
  private void saveTestsDetails(CSharpFile testFile, SensorContext context, UnitTestReport fileReport) throws TransformerException
  {
    ...
    {
      testCaseDetails.append("<testcase status=\"").append(detail.getStatus()).append("\" time=\"").append(detail.getTimeMillis())
                     .append("\" name=\"").append(detail.getName()).append("\"").append("\" asserts=\"").append(detail.getCountAsserts())
                     .append("\"");
      ...
      if (isError || (detail.getStatus() == TestStatus.FAILED))
      {
        testCaseDetails.append(">").append(isError ? "<error message=\"" : "<failure message=\"")
                       ...
                       .append(detail.getStackTrace()).append("]]>").append(isError ? "</error>" : "</failure>").append("</testcase>");
      }
      else
      {
   ...
   * @param value
   */
  private void saveFileMeasure(CSharpFile testFile, SensorContext context, UnitTestReport fileReport, Metric metric, double value)
  {
