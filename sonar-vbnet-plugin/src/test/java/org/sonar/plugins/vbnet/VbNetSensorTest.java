...
public class VbNetSensorTest {
  ...
  public void prepare() throws Exception {
    ...
    sensor = new VbNetSensor(reportPathCollector, protobufDataImporter, roslynDataImporter);
  }

  private void addFileToFs() {
    DefaultInputFile inputFile = new TestInputFileBuilder("mod", "file.vb").setLanguage(VbNetPlugin.LANGUAGE_KEY).build();
  ...
  public void noProtobufFilesShouldNotFail() {
    ...
    tester.setActiveRules(new ActiveRulesBuilder()
      .create(RuleKey.of(VbNetPlugin.REPOSITORY_KEY, "S1186"))
      .activate()
      .create(RuleKey.of(VbNetPlugin.REPOSITORY_KEY, "[parameters_key]"))
      .activate()
    ...
    // {"sonaranalyzer-vbnet" = [vbnet:S1186, vbnet:[parameters_key]], "foo" = [roslyn.foo:custom-roslyn]}
    ImmutableMap<String, List<RuleKey>> expectedMap = ImmutableMap.of(
      "sonaranalyzer-vbnet", ImmutableList.of(RuleKey.of("vbnet", "S1186"), RuleKey.of("vbnet", "[parameters_key]")),
