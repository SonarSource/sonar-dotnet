...
public class FxCopMojo extends AbstractDotNetMojo
{
   ...
   * 
   * @parameter expression="${fxcop.ignore.generated.code}" alias="ignoreGeneratedCode" default-value="true"
   */
  ...
  protected void executeProject(VisualStudioProject visualProject) throws MojoFailureException, MojoExecutionException
  {
    ...
    {
      getLog().info("No FxCop report generated for the test project " + visualProject.getName());
      return;
    }
  ...
  protected void prepareFxCopProject() throws MojoExecutionException
  {
    if (fxCopConfigFile == null)
    {
      ...
      fxCopConfigFile = extractResource(reportDirectory, DEFAULT_FX_COP_PROJECT, DEFAULT_FX_COP_PROJECT, "fxcop project");
    }
    else
    {
      if (!fxCopConfigFile.exists())
      {
        throw new MojoExecutionException("Could not find the fxcop project file: " + fxCopConfigFile);
      }
    }
