...
namespace Namespace
{
    public class Class1
    {
            ...
            float floatArg, int intArg, uint uintArg, long longArg, ulong ulongArg, short shortArg, ushort ushortArg)
        {
            ...
            Bar((byte) 1);          // %1 := __id [ const ]
                                    // %2 := Namespace.Class1.Bar(object) [ this %1 ]
            Bar((sbyte) 1);         // %3 := __id [ const ]
                                    // %4 := Namespace.Class1.Bar(object) [ this %3 ]
            Bar('c');               // %5 := Namespace.Class1.Bar(object) [ this const ]
                                    // %6 := __id [ const ]
            Bar((decimal) 1);       // %7 := Namespace.Class1.Bar(object) [ this %6 ]
                                    // %8 := __id [ const ]
            Bar((double) 1);        // %9 := Namespace.Class1.Bar(object) [ this %8 ]
                                    // %10 := __id [ const ]
            Bar((float) 1);         // %11 := Namespace.Class1.Bar(object) [ this %10 ]
                                    // %12 := __id [ const ]
            Bar((int) 1);           // %13 := Namespace.Class1.Bar(object) [ this %12 ]
                                    // %14 := __id [ const ]
            Bar((uint) 1);          // %15 := Namespace.Class1.Bar(object) [ this %14 ]
                                    // %16 := __id [ const ]
            Bar((long) 1);          // %17 := Namespace.Class1.Bar(object) [ this %16 ]
                                    // %18 := __id [ const ]
            Bar((ulong) 1);         // %19 := Namespace.Class1.Bar(object) [ this %18 ]
                                    // %20 := __id [ const ]
            Bar((short) 1);         // %21 := Namespace.Class1.Bar(object) [ this %20 ]
                                    // %22 := __id [ const ]
            Bar((ushort) 1);        // %23 := Namespace.Class1.Bar(object) [ this %22 ]

            Bar(boolArg);           // %24 := Namespace.Class1.Bar(object) [ this const ]
            Bar(byteArg);           // %25 := Namespace.Class1.Bar(object) [ this const ]
            Bar(sbyteArg);          // %26 := Namespace.Class1.Bar(object) [ this const ]
            Bar(charArg);           // %27 := Namespace.Class1.Bar(object) [ this const ]
            Bar(decimalArg);        // %28 := Namespace.Class1.Bar(object) [ this const ]
            Bar(doubleArg);         // %29 := Namespace.Class1.Bar(object) [ this const ]
            Bar(floatArg);          // %30 := Namespace.Class1.Bar(object) [ this const ]
            Bar(intArg);            // %31 := Namespace.Class1.Bar(object) [ this const ]
            Bar(uintArg);           // %32 := Namespace.Class1.Bar(object) [ this const ]
            Bar(longArg);           // %33 := Namespace.Class1.Bar(object) [ this const ]
            Bar(ulongArg);          // %34 := Namespace.Class1.Bar(object) [ this const ]
            Bar(shortArg);          // %35 := Namespace.Class1.Bar(object) [ this const ]
            Bar(ushortArg);         // %36 := Namespace.Class1.Bar(object) [ this const ]
        }
