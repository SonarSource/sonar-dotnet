...
namespace SonarAnalyzer.Rules.CSharp
{
    ...
    public class GenericTypeParameterInOut : SonarDiagnosticAnalyzer
    {
        ...
        private static void ReportIssue(ITypeParameterSymbol typeParameter, VarianceKind variance, SyntaxNodeAnalysisContext context)
        {
            ...
            {
                context.CheckReportDiagnostic(Diagnostic.Create(rule, location, "in", typeParameter.Name, "contravariant"));
                return;
            }

            if (variance == VarianceKind.Out)
            {
                context.CheckReportDiagnostic(Diagnostic.Create(rule, location, "out", typeParameter.Name, "covariant"));
            }
        }
