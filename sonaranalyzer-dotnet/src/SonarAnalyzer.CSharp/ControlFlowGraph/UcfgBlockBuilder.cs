...
namespace SonarAnalyzer.ControlFlowGraph.CSharp
{
    internal class UcfgBlockBuilder
    {
        ...
        {
            var ucfgBlock = CreateBlockWithId(blockIdProvider.GetOrAdd(block));

            ucfgBlock.Instructions.AddRange(block.Instructions.SelectMany(instructionBuilder.Create).WhereNotNull());
            ...
            if (ucfgBlock.TerminatorCase == BasicBlock.TerminatorOneofCase.None)
            {
                ...
                ucfgBlock.Jump = CreateJump(block.SuccessorBlocks.Select(blockIdProvider.GetOrAdd).ToArray());
            }
        ...
        {
            var ucfgBlock = CreateBlockWithId(blockIdProvider.GetOrAdd(new TemporaryBlock()));
