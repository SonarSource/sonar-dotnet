...
namespace SonarAnalyzer.Rules
{
    ...
    public class TestLocationAnalyzer : TestLocationAnalyzerBase
    {
        ...
        {
            var collector = new ClassStatementCollector();
            collector.Visit(syntaxTree.GetRoot());

            return collector.ClassStatements.Select(classStatement => semanticModel.GetDeclaredSymbol(classStatement));
        }

        private class ClassStatementCollector : VisualBasicSyntaxWalker
        {
            public List<ClassStatementSyntax> ClassStatements { get; } = new List<ClassStatementSyntax>();
