...
namespace SonarAnalyzer.Helpers
{
    public static class AnalysisContextExtensions
    {
        ...
        {
            TryReportDiagnostic(context.Node.SyntaxTree, diagnostic, d => context.ReportDiagnostic(d));
        }

        public static void ReportDiagnosticWhenActive(this SyntaxTreeAnalysisContext context, Diagnostic diagnostic)
        {
            TryReportDiagnostic(context.Tree, diagnostic, d => context.ReportDiagnostic(d));
        }

        public static void ReportDiagnosticWhenActive(this CompilationAnalysisContext context, Diagnostic diagnostic)
        {
            TryReportDiagnostic(context.Compilation.SyntaxTrees.FirstOrDefault(), diagnostic,
        ...
        {
            TryReportDiagnostic(context.Symbol.Locations.FirstOrDefault(l => l.SourceTree != null)?.SourceTree, diagnostic,
                d => context.ReportDiagnostic(d));
        }

        public static void ReportDiagnosticWhenActive(this CodeBlockAnalysisContext context, Diagnostic diagnostic)
        {
            TryReportDiagnostic(context.CodeBlock.SyntaxTree, diagnostic, d => context.ReportDiagnostic(d));
        }

        private static void TryReportDiagnostic(SyntaxTree tree, Diagnostic diagnostic, Action<Diagnostic> report)
        {
