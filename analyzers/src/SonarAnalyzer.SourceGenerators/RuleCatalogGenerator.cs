...
public class RuleCatalogGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // No RSPEC files means this generator is running in a project that does not carry a rule catalog (e.g. SonarAnalyzer.Core).
        var qualityProfile = RspecFiles(context, "qualityprofile").Collect().Select((x, ct) => CancelableExecute(() => x.IsDefaultOrEmpty ? [] : ParseSonarWay(x.Single().GetText().ToString()), ct));
        ...
        context.RegisterSourceOutput(source, (context, source) =>
            {
                ...
                {
                    context.AddSource("RuleCatalog.g.cs", source);
                }
            });
    }
    ...
    {
        if (rulesArguments.IsDefaultOrEmpty)
        {
            return null;
        }
