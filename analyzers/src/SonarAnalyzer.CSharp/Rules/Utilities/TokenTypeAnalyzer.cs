        ...
        internal sealed class TokenClassifier : TokenClassifierBase
        {
            ...
            protected override TokenTypeInfo.Types.TokenInfo ClassifyIdentifier(SyntaxToken token) =>
                token.Parent switch
                {
                    ...
                    var x when FunctionPointerUnmanagedCallingConventionSyntaxWrapper.IsInstance(x) => null,
                    var x when TupleElementSyntaxWrapper.IsInstance(x) => null,
                    var x when LocalFunctionStatementSyntaxWrapper.IsInstance(x) => null,
                    var x when SingleVariableDesignationSyntaxWrapper.IsInstance(x) => null,
