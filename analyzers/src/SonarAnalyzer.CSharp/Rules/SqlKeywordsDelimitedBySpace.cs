...
public sealed class SqlKeywordsDelimitedBySpace : SonarDiagnosticAnalyzer
{
    ...
    private static readonly IList<string> SqlStartQueryKeywords =
    [
        "ALTER",
        "BULK INSERT",
        "CREATE",
    ...
    private sealed class StringConcatenationWalker : SafeCSharpSyntaxWalker
    {
        ...
        private static bool IsInvalidCombination(char first, char second)
        {
            ...
            // Brackets delimit a SQL identifier (e.g. "[TableName]"); no space is required around them.
            if ((first == '[' && char.IsLetterOrDigit(second)) || (char.IsLetterOrDigit(first) && second == ']'))
            {
                return false;
            }
