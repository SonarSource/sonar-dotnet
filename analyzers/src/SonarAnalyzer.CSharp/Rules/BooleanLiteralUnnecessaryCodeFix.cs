...
public sealed class BooleanLiteralUnnecessaryCodeFix : SonarCodeFix
{
    ...
    public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(BooleanLiteralUnnecessary.DiagnosticId);
    ...
    protected override Task RegisterCodeFixesAsync(SyntaxNode root, SonarCodeFixContext context)
    {
        ...
        var syntaxNode = ((ExpressionSyntax)node).WithoutEnclosingParentheses;
        ...
        if (syntaxNode is not LiteralExpressionSyntax)
        {
            return Task.CompletedTask;
        }
    ...
    private static void RegisterBinaryExpressionRemoval(SonarCodeFixContext context, SyntaxNode root, LiteralExpressionSyntax literal, BinaryExpressionSyntax binaryParent)
    {
        ...
        context.RegisterCodeFix(
            ...
            x =>
            {
                var newExpression = NegatedExpression(otherNode).WithAdditionalAnnotations(Simplifier.Annotation);
    ...
    private static void RegisterConditionalExpressionRewrite(SonarCodeFixContext context, SyntaxNode root, LiteralExpressionSyntax literal, ConditionalExpressionSyntax conditionalParent) =>
        context.RegisterCodeFix(
            Title,
            x => Task.FromResult(RewriteConditional(context.Document, root, literal, conditionalParent)),
            context.Diagnostics);

    private static void RegisterBooleanInversion(SonarCodeFixContext context, SyntaxNode root, LiteralExpressionSyntax literal) =>
        context.RegisterCodeFix(
            Title,
            x => Task.FromResult(RemovePrefixUnary(context.Document, root, literal)),
            context.Diagnostics);
    ...
    private static void RegisterConditionalExpressionRemoval(SonarCodeFixContext context, SyntaxNode root, ConditionalExpressionSyntax conditional) =>
        context.RegisterCodeFix(
            Title,
            x => Task.FromResult(RemoveConditional(context.Document, root, conditional)),
            context.Diagnostics);

    private static void RegisterBinaryExpressionReplacement(SonarCodeFixContext context, SyntaxNode root, SyntaxNode node, BinaryExpressionSyntax binary) =>
        context.RegisterCodeFix(
            Title,
            x =>
            {
                var keepThisNode = FindNodeToKeep(binary).WithAdditionalAnnotations(Simplifier.Annotation);
                var newRoot = root.ReplaceNode(node, keepThisNode
                    .WithAdditionalAnnotations(Formatter.Annotation));
                return Task.FromResult(context.Document.WithSyntaxRoot(newRoot));
            },
            context.Diagnostics);
    ...
    private static SyntaxNode FindNodeToKeep(BinaryExpressionSyntax binary) =>
        NodeToKeepForLogicalOperators(binary)
        ?? NodeToKeepForEqualityOperators(binary)
    ...
    private static SyntaxNode NodeToKeepForLogicalOperators(BinaryExpressionSyntax binary)
    {
    ...
    private static SyntaxNode NodeToKeepForEqualityOperators(BinaryExpressionSyntax binary)
    {
        // ==/!= both sides booleans
    ...
    private static Document RemoveConditional(Document document, SyntaxNode root, ConditionalExpressionSyntax conditional)
    {
        ...
        else
        {
            var newRoot = root.ReplaceNode(
                conditional,
                NegatedExpression(conditional.Condition).WithAdditionalAnnotations(Formatter.Annotation));
            return document.WithSyntaxRoot(newRoot);
        }
    }
    ...
    private static Document RewriteConditional(Document document, SyntaxNode root, SyntaxNode node, ConditionalExpressionSyntax conditional)
    {
        var whenTrue = conditional.WhenTrue.WithoutEnclosingParentheses;
        if (whenTrue.Equals(node) && node.IsTrue())
        {
        ...
        if (whenTrue.Equals(node) && node.IsFalse())
        {
            var newRoot = ReplaceExpressionWithBinary(
                conditional,
                root,
                SyntaxKind.LogicalAndExpression,
                NegatedExpression(conditional.Condition),
                AddParenthesis(conditional.WhenFalse));
        ...
        if (whenFalse.Equals(node) && node.IsTrue())
        {
            var newRoot = ReplaceExpressionWithBinary(
                conditional,
                root,
        ...
        if (whenFalse.Equals(node) && node.IsFalse())
        {
            var newRoot = ReplaceExpressionWithBinary(
                conditional,
                root,
    ...
    private static ExpressionSyntax NegatedExpression(ExpressionSyntax expression) =>
