...
public class TooManyParameters : TooManyParametersBase<SyntaxKind, ParameterListSyntax>
{
    ...
    // A primary constructor is declared by the type declaration itself, so the base implementation would resolve the type instead of the constructor.
    protected override IMethodSymbol DeclaredConstructor(SyntaxNode node, SemanticModel model) =>
        node is TypeDeclarationSyntax typeDeclaration
            ? typeDeclaration.PrimaryConstructor(model)
            : base.DeclaredConstructor(node, model);
