...
public sealed class ClearTextProtocolsAreSensitive : SonarDiagnosticAnalyzer
{
    ...
    private static readonly string[] CommonlyUsedExampleDomains = ["example.com", "example.org", "example.net", "test.com"];
    private static readonly string[] LocalhostAddresses = ["localhost", "127.0.0.1", "::1", "169.254.169.254"];

    private static readonly CSharpObjectInitializationTracker ObjectInitializationTracker =
        new(
            x => x is bool value && value,
            ImmutableArray.Create(KnownType.System_Net_Mail_SmtpClient, KnownType.System_Net_FtpWebRequest),
            x => x == EnableSslName);

    private static readonly TimeSpan RegexTimeout = TimeSpan.FromMilliseconds(250);
    private static readonly string ValidServerPattern = BuildValidServerPattern();
    private static readonly Regex HttpRegex = CompileRegex(@$"^http:\/\/(?!{ValidServerPattern}).");
    private static readonly Regex FtpRegex = CompileRegex(@$"^ftp:\/\/.*@(?!{ValidServerPattern})");
    private static readonly Regex TelnetRegex = CompileRegex(@$"^telnet:\/\/.*@(?!{ValidServerPattern})");
    private static readonly Regex TelnetRegexForIdentifier = CompileRegex(@"Telnet(?![a-z])", false);
    private static readonly Regex ValidServerRegex = CompileRegex($"^({ValidServerPattern})$");
    ...
    {
        if (UrlAnalyzer.UnsafeProtocol(c.Node, c.Model) is { } unsafeProtocol)
        {
            c.ReportIssue(DefaultRule, c.Node, unsafeProtocol, RecommendedProtocols[unsafeProtocol]);
        }
    }

    private static bool IsServerSafe(IObjectCreation objectCreation, SemanticModel model) =>
        objectCreation.ArgumentList?.Arguments.Count > 0
        && ValidServerRegex.SafeIsMatch(UrlAnalyzer.Text(objectCreation.ArgumentList.Arguments[0].Expression, model));

    private static string BuildValidServerPattern()
    {
    ...
    {
        private static readonly KnownType[] AttributesWithNamespaceParameter =
        [
        ...
        internal static bool IsNamespace(SemanticModel model, SyntaxNode node) =>
            node switch
            {
                ...
                AssignmentExpressionSyntax assignmentExpression =>
                    assignmentExpression.Left.WithoutEnclosingParentheses is IdentifierNameSyntax identifierName && TokenContainsNamespace(identifierName.Identifier),
    ...
    private static class UrlAnalyzer
    {
        ...
        internal static string Text(SyntaxNode node, SemanticModel model)
        {
