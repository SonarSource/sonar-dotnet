...
public sealed class DisposableNotDisposed : SonarDiagnosticAnalyzer
{
    ...
    protected override void Initialize(SonarAnalysisContext context) =>
        context.RegisterSymbolAction(
            c =>
            {
                ...
                var typesDeclarationsAndSemanticModels = namedType.DeclaringSyntaxReferences.Select(x => CreateNodeAndModel(c, x)).ToList();
    ...
    {
        var descendantNodes = DescendantNodes(namedType, typeDeclaration).ToArray();
        ...
        {
            var trackedVariables = declaration.Variables.Where(x => x.Initializer is not null && IsInstantiation(x.Initializer.Value, model));
            foreach (var variableNode in trackedVariables)
            {
                disposableObjects.Add(new NodeAndSymbol(variableNode, model.GetDeclaredSymbol(variableNode)));
            }
        }
    }
    ...
    private static void TrackAssignmentsToLocalsAndPrivateFields(INamedTypeSymbol namedType,
                                                                 ...
                                                                 ISet<NodeAndSymbol> disposableObjects)
    {
        var simpleAssignments = DescendantNodes(namedType, typeDeclaration).Where(x => x.IsKind(SyntaxKind.SimpleAssignmentExpression)).Cast<AssignmentExpressionSyntax>();

        foreach (var simpleAssignment in simpleAssignments)
        {
            if (!IsNodeInsideUsingStatement(simpleAssignment)
                && IsInstantiation(simpleAssignment.Right, model)
                && model.GetSymbolInfo(simpleAssignment.Left).Symbol is { } referencedSymbol
    ...
    private static bool IsNodeInsideUsingStatement(SyntaxNode node)
    {
        ...
        var isPartOfUsingStatement = ancestors
            ...
            .Any(x => (x.Expression is not null && x.Expression.DescendantNodesAndSelf().Contains(node)) || (x.Declaration is not null && x.Declaration.DescendantNodesAndSelf().Contains(node)));
    ...
    private static void ExcludeDisposedAndClosedLocalsAndPrivateFields(SyntaxNode typeDeclaration, SemanticModel model, ISet<ISymbol> possiblyDisposed)
    {
        ...
        foreach (var invocationOrConditionalAccess in invocationsAndConditionalAccesses)
        {
            ...
            if (name is not null
                ...
                && model.GetSymbolInfo(expression).Symbol is { } referencedSymbol
                && IsLocalOrPrivateField(referencedSymbol))
            {
    ...
    private static void ExcludeReturnedPassedAndAliasedLocalsAndPrivateFields(SyntaxNode typeDeclaration, SemanticModel model, ISet<ISymbol> possiblyDisposed)
    {
        var identifiersAndSimpleMemberAccesses = typeDeclaration
            .DescendantNodes()
            .Where(x => x.IsKind(SyntaxKind.IdentifierName) || x.IsKind(SyntaxKind.SimpleMemberAccessExpression));
        ...
        foreach (var identifierOrSimpleMemberAccess in identifiersAndSimpleMemberAccesses)
        {
            ...
            if (IsStandaloneExpression(expression)
                && model.GetSymbolInfo(identifierOrSimpleMemberAccess).Symbol is { } referencedSymbol
                && IsLocalOrPrivateField(referencedSymbol))
            {
                possiblyDisposed.Add(referencedSymbol);
            }
    ...
    private static bool IsStandaloneExpression(ExpressionSyntax expression) =>
        expression.Parent is not ExpressionSyntax
