...
public sealed class HardcodedIpAddress : HardcodedIpAddressBase<SyntaxKind, LiteralExpressionSyntax>
{
    ...
    protected override string AssignedVariableName(SyntaxNode stringLiteral) =>
        stringLiteral.FirstAncestorOrSelf<SyntaxNode>(IsVariableIdentifier)?.ToString();

    private static bool IsVariableIdentifier(SyntaxNode node) =>
        node is StatementSyntax or VariableDeclaratorSyntax or ParameterSyntax;
}
