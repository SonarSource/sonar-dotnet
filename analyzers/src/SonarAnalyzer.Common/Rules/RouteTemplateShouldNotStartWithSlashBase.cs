    ...
    where TSyntaxKind : struct
{
    ...
    private void ReportIssues(SonarSymbolReportingContext context, INamedTypeSymbol controllerSymbol, ConcurrentStack<ControllerActionInfo> actions)
    {
        ...
        {
            var classLocations = controllerSyntaxDeclarations.Select(x => Language.Syntax.NodeIdentifier(x)?.GetLocation());
            var secondaryLocations = FindActionSecondaryLocations(actions).Concat(classLocations.Skip(1));
            context.ReportIssue(Language.GeneratedCodeRecognizer, Diagnostic.Create(Rule, classLocations.First(), secondaryLocations, issueMessage));
        }
        ...
        IEnumerable<Location> FindActionSecondaryLocations(ConcurrentStack<ControllerActionInfo> actions) =>
            actions
                .Where(x => !x.Action.GetFirstSyntaxRef().SyntaxTree.IsGenerated(Language.GeneratedCodeRecognizer)) // We don't report on autogenerated actions
                .SelectMany(x => x.RouteParameters)
                .Select(x => x.Key);
    }
