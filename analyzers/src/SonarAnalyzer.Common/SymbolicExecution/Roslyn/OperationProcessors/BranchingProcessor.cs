...
{
    protected abstract SymbolicConstraint BoolConstraintFromOperation(ProgramState state, T operation, bool isLoopCondition);
    ...
    protected override ProgramState[] Process(SymbolicContext context, T operation)
    {
        ...
        if (BoolConstraintFromOperation(state, operation, context.IsLoopCondition) is { } constraint)
        {
