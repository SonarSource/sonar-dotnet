...
public abstract class ExecutingSqlQueriesBase<TSyntaxKind, TExpressionSyntax, TIdentifierNameSyntax> : SonarDiagnosticAnalyzer<TSyntaxKind>
    ...
    where TIdentifierNameSyntax : SyntaxNode
{
    ...
    protected abstract TExpressionSyntax ArgumentAtIndex(InvocationContext context, int index);
    protected abstract TExpressionSyntax ArgumentAtIndex(ObjectCreationContext context, int index);
    ...
    private void TrackObjectCreation(TrackerInput input, KnownType[] objectCreationTypes, int argumentIndex)
    {
        ...
        t.Track(
            ...
            x => IsTracked(ArgumentAtIndex(x, argumentIndex), x),
    ...
    private TrackerBase<TSyntaxKind, InvocationContext>.Condition MethodHasRawSqlQueryParameter() =>
        x =>
        {
            return Language.Syntax.NodeExpression(x.Node) is { } invocationExpression
                && x.Model.GetSymbolInfo(invocationExpression).Symbol is IMethodSymbol methodSymbol
    ...
    private TrackerBase<TSyntaxKind, InvocationContext>.Condition ArgumentAtIndexIsTracked(int index) =>
        x => IsTracked(ArgumentAtIndex(x, index), x);
}
