    ...
    where TParameterListSyntax : SyntaxNode
{
    ...
    private static readonly ImmutableArray<KnownType> DependencyInjectionAttributes = ImmutableArray.Create(
    ...
    protected virtual IMethodSymbol DeclaredConstructor(SyntaxNode node, SemanticModel model) =>
        model.GetDeclaredSymbol(node) as IMethodSymbol;
    ...
    protected bool VerifyCanBeChangedBySymbol(SyntaxNode node, SemanticModel model)
    {
        var declaredSymbol = model.GetDeclaredSymbol(node);
        var symbol = model.GetSymbolInfo(node).Symbol;
        if (declaredSymbol is null && symbol is null)
        {
            return false;
        }
        ...
        if (DeclaredConstructor(node, model) is { MethodKind: MethodKind.Constructor } constructor && IsDependencyInjected(constructor))
        {
            return false;   // The dependency injection container, and not a caller, provides the arguments.
        }

        if (declaredSymbol.IsStatic
            && ((declaredSymbol.IsExtern && declaredSymbol.HasAttribute(KnownType.System_Runtime_InteropServices_DllImportAttribute))
    ...
    // For plain constructor injection there is no per-parameter marker: the whole type is activated by the container.
    private static bool IsDependencyInjected(IMethodSymbol constructor) =>
        constructor.HasAttribute(KnownType.Microsoft_Extensions_DependencyInjection_ActivatorUtilitiesConstructorAttribute)
        || constructor.ContainingType.IsDependencyInjectionManagedType;
}
