...
public abstract class GeneratedCodeRecognizer
{
    ...
    /// <summary>
    /// Detects if the file has been generated by the IDE (VS/VSCode) for Razor.
    /// </summary>
    ...
    public static bool IsRazor(SyntaxTree tree) =>
        ...
        || (tree.EndsWith("ide.g.cs") && RazorPattern.SafeIsMatch(tree.FilePath)); // Performance: Check before regex with "ide.g.cs"

    public static bool IsCshtml(SyntaxTree tree) =>
        GeneratedCshtmlSuffixes.Any(tree.EndsWith)
        || (tree.EndsWith("ide.g.cs") && CshtmlPattern.SafeIsMatch(tree.FilePath)); // Performance: Check before regex with "ide.g.cs"
