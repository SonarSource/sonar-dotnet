...
'       ^^^^^^^^^^^^^^
    ...
    Private Sub CompoundAssignment_NotASet(ByVal dict As Dictionary(Of Integer, Integer))
        dict(0) = 0
        dict(0) += 1 ' Compliant
    End Sub

    Private Sub CompoundAssignment_Overwritten(ByVal dict As Dictionary(Of Integer, Integer))
        dict(0) += 1 ' Secondary
        dict(0) = 2  ' Noncompliant
    End Sub

    Private Sub CompoundAssignment_DoesNotStopTheScan(ByVal dict As Dictionary(Of Integer, Integer))
        dict(0) = 1  ' Secondary
        dict(1) += 1 ' Different key, no longer a barrier
        dict(0) = 2  ' Noncompliant
    End Sub

    Private Sub OnlyTheNearest_SetIsReported(ByVal dict As Dictionary(Of Integer, Integer))
        dict(0) += 1
        dict(0) += 2 ' Secondary
        dict(0) = 3  ' Noncompliant
    End Sub

    Private Sub UnrelatedAssignmentStopsTheScan(ByVal dict As Dictionary(Of Integer, Integer), ByVal other As Dictionary(Of Integer, Integer))
        dict(0) = 1
        other(0) = 1
        dict(0) = 2 ' FN - the scan stops at the unrelated collection
    End Sub
    ...
    Private Sub DictionaryAccessExpression_OhBoy(ByVal dict As Dictionary(Of String, List(Of Integer)))
        dict!a(0) = 1 ' Secondary
        dict!a(0) = 2 ' Noncompliant
    End Sub
...
Class NullReferenceReproducer
    ...
    Sub FooBar()
        Bar ' AD0001 NullReferenceException in FirstArgumentExpression
    End Sub
...
Public Class Repro_AD0001_4571

    Private fHandler As Action(Of Integer)

    Public Sub Method(Value As Integer)
        fHandler?(Value)
    End Sub
