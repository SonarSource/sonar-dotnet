...
public class SymbolicContextTest
{
    ...
    public void NullArgument_State_Throws()
    {
        ...
        var create = () => new SymbolicContext(block, new IOperationWrapperSonar(block.Operations[0]), null, false, false, 0, Array.Empty<ISymbol>());
    ...
    {
        var block = CreateBlock();
        var create = () => new SymbolicContext(block, new IOperationWrapperSonar(block.Operations[0]), ProgramState.Empty, false, false, 0, null);
    ...
    public void NullOperation_SetsOperationToNull() =>
        new SymbolicContext(null, default, ProgramState.Empty, false, false, 0, Array.Empty<ISymbol>()).Operation.Instance.Should().BeNull();
    ...
    public void PropertiesArePersisted()
    {
        ...
        var sut = new SymbolicContext(block, operation, state, true, true, 42, Array.Empty<ISymbol>());
    ...
    public void SetOperationConstraint_WithExistingValue()
    {
        ...
        var operation = new IOperationWrapperSonar(block.Operations[0]);
        var state = ProgramState.Empty.SetOperationValue(operation, SymbolicValue.Empty);
        var sut = new SymbolicContext(block, operation, state, false, false, 0, Array.Empty<ISymbol>());
    ...
    {
        var state = ProgramState.Empty;
        var sut = new SymbolicContext(null, default, state, false, false, 0, Array.Empty<ISymbol>());
