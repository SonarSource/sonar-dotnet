...
using NullabilityInfo = Microsoft.CodeAnalysis.NullabilityInfo;
...
public class TypeWrapStrategyTest
{
    ...
    [TestMethod]
    public void Generate_FromReturnsNullableWrapper()
    {
        var sut = new TypeWrapStrategy(
            typeof(NullabilityInfo),
            typeof(object),
            [
                new(typeof(NullabilityInfo).GetMember(nameof(NullabilityInfo.Annotation))[0], false, "AnnotationAccessor"),
            ]);
        var result = sut.Generate([]);
        result.Should().BeIgnoringLineEndings(
            """
