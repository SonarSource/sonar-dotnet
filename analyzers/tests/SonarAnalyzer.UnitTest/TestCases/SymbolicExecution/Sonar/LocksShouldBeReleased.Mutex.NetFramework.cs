...
namespace Net6
{
    ...
    {
        public static void AcquiredNotReleased(MutexSecurity mutexSecurity)
        {
            // Dispose does not release the resource
            var m0 = new Mutex(initiallyOwned: true, "foo", out var mutexWasCreated, mutexSecurity); // FN
            m0.Dispose();
        }

        public static void CompliantNoAcquire(MutexSecurity mutexSecurity)
        {
            var m2 = new Mutex(initiallyOwned: false, "foo", out var mutexWasCreated, mutexSecurity);
        }
